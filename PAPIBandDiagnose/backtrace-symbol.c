 ...
 * 	 	 * the second time we do the actual printing */
	...
	if (state == Print) {
		ret_buf = __libc_malloc(total + sizeof(char*)*naddr);
...
char **backtrace_symbols_change(void *const *buffer, int size)
{
	...
	char **final;
	char *f_strings;
	locations = __libc_malloc(sizeof(char**) * (stack_depth+1));
	bfd_init();
 ...
 * 	 * all of the strings */
	final = __libc_malloc(total + (stack_depth + 1) * sizeof(char*));
